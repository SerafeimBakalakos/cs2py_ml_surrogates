...
def train_ffnn(model_architecture, ffnn_model, model_params, train_pod_coeffs):
    ...
    ffnn_learning_rate = provide_learning_rate_schedule(
        ...
        num_epochs=ffnn_num_epochs, batch_size=ffnn_batch_size, num_training_samples=train_pod_coeffs.shape[0])
...
def run_main_script_body(settings: dict, durations: dict):
    ...
    path_train_pod_coeffs = settings["TrainPodCoeffsPath"]
    ...
    train_pod_coeffs = arrayIO.load_array2D(path_train_pod_coeffs, np_dtype)
    train_pod_coeffs = np.expand_dims(train_pod_coeffs, axis=1)

    # Read FFNN model architecture
    ...
    start = time.time_ns()
    print("***** Training FFNN *****")
    ffnn_history = train_ffnn(model_architecture, ffnn_model, train_model_params, train_pod_coeffs)
